...
fn test_go_artifact_caching() {
    TestRunner::default().build(
        ...
        |ctx| {
            assert_contains!(
                ctx.pack_stdout,
                "Installing Go 1.16.15 (linux-x86_64) from https://go.dev/dl/go1.16.15.linux-amd64.tar.gz",
            );
            let config = ctx.config.clone();
            ctx.rebuild(config, |ctx| {
                assert_contains!(ctx.pack_stdout, "Reusing Go 1.16.15 (linux-x86_64)");
            });
        },
    );
}
